...
namespace VCRDistortion {
    ...
    kernel void vcr_distortion(
        ...
        if (params.skip > 0.0) {
            ...
            shift = sin(frameUniforms.time * 10.0) * sin(frameUniforms.time * 0.5) * sin(frameUniforms.time * 1.0) *
                    0.005 * onOffValue;
            shift *= params.skip;
        ...
        if (params.scanlines_opacity > 0.0) {
            float scanline = sin((uv.y + frameUniforms.time * params.scanlines_speed * 0.01) * textureSize.y *
                                 params.scanlines_spacing * 0.5) *
                                 0.5 +
                             0.5;
        ...
        if (params.small_scanlines_opacity > 0.0) {
            float smallScanline = sin((uv.y - frameUniforms.time * params.small_scanlines_speed * 0.1) * textureSize.y *
                                      params.small_scanlines_proximity) *
                                      0.5 +
                                  0.5;
