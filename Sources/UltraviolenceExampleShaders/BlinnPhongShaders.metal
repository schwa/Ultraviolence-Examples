...
[[vertex]]
Fragment BlinnPhongVertexShader(
    Vertex in [[stage_in]],
    ...
    )
{
    ...
    const float4 modelVertex = transforms.modelViewMatrix * position;
    out.position = transforms.modelViewProjectionMatrix * position;
    out.modelPosition = float3(modelVertex) / modelVertex.w;
    out.interpolatedNormal = transforms.modelNormalMatrix * in.normal;
    out.textureCoordinate = in.textureCoordinate;
    return out;
}

[[fragment]]
float4 BlinnPhongFragmentShader(
...
)
{
    ...
    float3 ambientColor;
    if (material.ambientSource == texture) {
        ambientColor = material.ambientColor.xyz;
    }
    else {
        ambientColor = material.ambientTexture.sample(material.ambientSampler, in.textureCoordinate).rgb;
    }

    float3 diffuseColor;
    if (material.diffuseSource == texture) {
        diffuseColor = material.diffuseColor.xyz;
    }
    else {
        diffuseColor = material.diffuseTexture.sample(material.diffuseSampler, in.textureCoordinate).rgb;
    }

    float3 specularColor;
    if (material.specularSource == texture) {
        specularColor = material.specularColor.xyz;
    }
    else {
        specularColor = material.specularTexture.sample(material.specularSampler, in.textureCoordinate).rgb;
    }
